...
import { AdditionalScopes, RouterEvent, VirtualPageStack, VirtualPageStackAction } from '../models/router-dispatch.interface';
import { getHostName } from '../utils/get-host-name';
import { replaceTokensWithValuesFromParamMap } from '../utils/replace-tokens-with-values-from-param-map';
...
export class RouterDispatchService {
  ...
  initialize() {
    ...
    const routerEvents$: Observable<RouterEvent> = this.router.events.pipe(
      ...
      map<any, RouterEvent>(
        (
          [location, additionalScopes, selectedItems, shouldTrack, shouldIncludeAppScope]: [string, AdditionalScopes, SelectedItems, boolean, boolean]
        ) => {
          const currentHostName = getHostName(this.windowService.url, location);
  ...
  getPrettyRouteName = (router: Router): string | undefined => {
    const snapshot = getSnapshotHierarchyAsArray([router.routerState.root.snapshot])
      .reduce((acc, snapshotFragment) => {
        const nextPath = snapshotFragment.routeConfig && snapshotFragment.routeConfig.path ? '/' + snapshotFragment.routeConfig.path : '';
      ...
      }, { paramMap: convertToParamMap({}), routeConfig: { path: '' } }) as ActivatedRouteSnapshot;

    if (snapshot.routeConfig && snapshot.paramMap.keys.length >= 1) {
      if (snapshotHasReplaceParamTokens(snapshot)) {
        return replaceTokensWithValuesFromParamMap(
